...
<div class="btn" style="border: none; width: 100%; height: 150px; background-color: #007A5E; color: white;">
  ...
  <div v-if="modalVisible2" class="modals2">
    <div class="details2 animate__animated animate__fadeInDown">
      ...
      <form @submit.prevent="createTypeuse">
        <div class="inp-field">
...
export default {
  ...
  methods: {
    ...
    async createTypeuse() {
      try {
        const response = await axios.post('http://192.168.100.116:3000/type_user', {
    ...
    async getTypeuse() {
      try {
        const response = await axios.get('http://192.168.100.116:3000/type_user'); // Appeler l'API GET
        this.type_user = response.data;
        console.log(response.data);
      } catch (error) {
        console.error('Erreur lors de la récupération des type_user :', error);
      }
    },
  }
};
