...
export default {
    ...
    methods: {
        ...
        async deletePersonnel() {
            try {
                // Récupérer le matricule depuis le localStorage
                const matricule = localStorage.getItem('matricule');

                // Effectuer la suppression avec le matricule depuis le localStorage
                const response = await axios.delete(`http://localhost:3000/personnel/${matricule}`);
                console.log(response.data);

                // Supprimer la réalisation de la liste des réalisations
